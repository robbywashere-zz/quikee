const { User, Account, Post } = require('../../models');
...
describe('User model', ()=>{
  ...
  it('- should respond to #.getPosts(), and have-many Posts through Accounts', async ()=>{
  
    const { account, user } = await UserAccountFactory();

    const account2 = await AccountFactory();

    const p1 = await Post.create({ AccountId: account.id });

    const p2 = await Post.create({ AccountId: account.id });

    const p3 = await Post.create({ AccountId: account2.id });

    await account2.addUser(user);

    const userPosts = await user.getPosts();
      
    assert.equal(userPosts.length, 3);
